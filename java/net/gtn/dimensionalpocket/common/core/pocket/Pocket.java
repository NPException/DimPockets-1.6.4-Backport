...
public class Pocket {
    ...
    public int getExternalLight() {
        if (isSourceBlockPlaced()) {
            ...
            TileEntity tileEntity = world.getBlockTileEntity(blockCoords.getX(), blockCoords.getY(), blockCoords.getZ());
    ...
    public void generatePocketRoom() {
        ...
        for (int x = 0; x < 16; x++)
            for (int y = 0; y < 16; y++)
                for (int z = 0; z < 16; z++) {
                    ...
                    extendedBlockStorage.setExtBlockID(x, y, z, ModBlocks.dimensionalPocketFrame.blockID);
        ...
        generated = world.getBlockId((chunkCoords.getX() * 16) + 1, chunkCoords.getY() * 16, (chunkCoords.getZ() * 16) + 1) == ModBlocks.dimensionalPocketFrame.blockID;
    }
    ...
    public boolean teleportFrom(EntityPlayer entityPlayer) {
        ...
        if (isSourceBlockPlaced()) {
            ...
            if (teleportSide != TeleportDirection.UNKNOWN) {
                ...
                ChatMessageComponent comp = ChatMessageComponent.createFromTranslationKey("info.trapped2");
                comp.setItalic(Boolean.TRUE);
                entityPlayer.addChatMessage(comp.toString());
            }
        } else {
        	ChatMessageComponent comp = ChatMessageComponent.createFromTranslationKey("info.trapped");
            comp.setItalic(Boolean.TRUE);
            entityPlayer.addChatMessage(comp.toString());
        }
    ...
    public boolean isSourceBlockPlaced() {
        World world = MinecraftServer.getServer().worldServerForDimension(blockDim);
        int blockId = world.getBlockId(blockCoords.getX(), blockCoords.getY(), blockCoords.getZ());
        return (blockId == ModBlocks.dimensionalPocket.blockID);
    }
